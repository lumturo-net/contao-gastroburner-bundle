...
use Contao\Date as Date;
use Contao\Encryption as Encryption;
use Contao\Email as Email;
use Contao\Environment as Environment;
use Contao\Files as Files;
use Contao\FilesModel as FilesModel;
use Contao\Folder as Folder;
use Contao\FrontendTemplate as FrontendTemplate;
use Contao\Idna as Idna;
use Contao\MemberModel as MemberModel;
use Contao\NewsletterChannelModel as NewsletterChannelModel;
use Contao\OptInModel as OptInModel;
use Contao\FormPassword as FormPassword;
use Contao\FrontendUser as FrontendUser;
use Contao\ModuleModel;
use Contao\PageModel as PageModel;
use Contao\System as System;
use Contao\StringUtil as StringUtil;
use Contao\Versions as Versions;
use Patchwork\Utf8;

/**
 * Front end module "registration". Angepasst für Gastroburner
 *
 * @author Rolf Staege <rolf.staege@lumturo.net>
 */
class GastroburnerRegistrationModule extends \Contao\ModuleRegistration
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_member_gastroburner_default';
	...
	public function generate()
	{
		if (TL_MODE == 'BE') {
			...
			$objTemplate->wildcard = '### ' . Utf8::strtoupper($GLOBALS['TL_LANG']['FMD']['gastroburnerregisterform'][0]) . '  ###';
		...
		// $objOriginalRegistrationModule = ModuleModel::findOneBy(array('type=?', 'editable!=?'), array('registration', ''), array());
		// $this->editable = \Contao\StringUtil::deserialize($objOriginalRegistrationModule->editable);
		// $this->reg_activate = $objOriginalRegistrationModule->reg_activate;
		// $this->reg_text = $objOriginalRegistrationModule->reg_text;
		// $this->objModel = $objOriginalRegistrationModule; // für jumpTo
	...
	protected function compile()
	{
		...
		// if ($this->memberTpl != '') {
		// 	$this->Template = new \Contao\FrontendTemplate($this->memberTpl);
		...
		// Build the form
		// Merke mir die Felder mit Namen, um sie im Template in Reihenfolge ausgeben zu können
		// $this->Template->named_fields = array();
		$arrNamedFields = array();
		$arrJobFields = array();
		foreach ($this->editable as $field) {
			...
			$arrNamedFields[$field] = $temp; // merke mir hier die Felder nach namen
			// $this->Template->named_fields[$field] = $temp; // merke mir hier die Felder nach namen
		...
		foreach (array('restaurant', 'cook', 'kitchen', 'systemg', 'hotelcleaner', 'hotelmanager', 'gastro') as $strField) {
			$arrJobFields[$strField] = $arrNamedFields[$strField];
			unset($arrNamedFields[$strField]);
		}
		$this->Template->named_fields = $arrNamedFields; // merke mir hier die Felder nach namen
		$this->Template->named_job_fields = $arrJobFields; // merke mir hier die Felder nach namen
...
class_alias(GastroburnerRegistrationModule::class, 'GastroburnerRegistrationModule');
