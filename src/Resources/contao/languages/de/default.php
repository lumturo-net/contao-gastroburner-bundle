<?php

$GLOBALS['TL_LANG']['MOD']['tl_question'][0] = 'Fragen';
$GLOBALS['TL_LANG']['MOD']['tl_apply'][0] = 'Bewerbungen';
$GLOBALS['TL_LANG']['MOD']['tl_company'][0] = 'Ausbildungsbetriebe';

$GLOBALS['TL_LANG']['tl_settings']['gastroburner_config'] = 'Gastroburner-Config';
$GLOBALS['TL_LANG']['tl_settings']['gastroburner_applyform_page'][0] = 'Zielseite für Bewerbungen';
$GLOBALS['TL_LANG']['tl_settings']['gastroburner_applyform_bcc'][0] = 'BCC-Empfänger für Bewerbungen';

// $GLOBALS['TL_LANG']['gastroburner']['category']['STRESS'] = array('STRESSRESISTENZ');
$GLOBALS['TL_LANG']['tl_question']['question_legend'] = 'Frage';
$GLOBALS['TL_LANG']['tl_question']['question'] = array('Frage');
$GLOBALS['TL_LANG']['tl_question']['type'] = array('Fragen-Typ');
$GLOBALS['TL_LANG']['tl_question']['answer_legend'] = 'Antworten';
$GLOBALS['TL_LANG']['tl_question']['answer0'] = array('Antwort 0', 'Wenn leer, wird diese Antwort nicht mit ausgegeben.');
$GLOBALS['TL_LANG']['tl_question']['answer1'] = array('Antwort 1', 'Wenn leer, wird diese Antwort nicht mit ausgegeben.');
$GLOBALS['TL_LANG']['tl_question']['answer2'] = array('Antwort 2', 'Wenn leer, wird diese Antwort nicht mit ausgegeben.');
$GLOBALS['TL_LANG']['tl_question']['answer3'] = array('Antwort 3', 'Wenn leer, wird diese Antwort nicht mit ausgegeben.');

$GLOBALS['TL_LANG']['tl_apply']['name_legend'] = 'Name';
$GLOBALS['TL_LANG']['tl_apply']['job_legend'] = 'Berufe';
$GLOBALS['TL_LANG']['tl_apply']['vorname'] = array('Vorname');
$GLOBALS['TL_LANG']['tl_apply']['name'] = array('Name');
$GLOBALS['TL_LANG']['tl_apply']['email'] = array('E-Mail-Adresse');
$GLOBALS['TL_LANG']['tl_apply']['cook'] = array('Koch/Köchin');
$GLOBALS['TL_LANG']['tl_apply']['kitchen'] = array('Fachkraft Küche');
$GLOBALS['TL_LANG']['tl_apply']['systemg'] = array('Fachmann/-frau für Systemgastronomie');
$GLOBALS['TL_LANG']['tl_apply']['restaurant'] = array('Fachmann/-frau für Restaurants und Veranstaltungsgastronomie');
$GLOBALS['TL_LANG']['tl_apply']['hotelcleaner'] = array('Hotelfachmann/-frau');
$GLOBALS['TL_LANG']['tl_apply']['hotelmanager'] = array('Kaufmann/-frau für Hotelmanagement');
$GLOBALS['TL_LANG']['tl_apply']['gastro'] = array('Fachkraft für Gastronomie');
$GLOBALS['TL_LANG']['tl_apply']['beschreibung_legend'] = 'Beschreibung';
$GLOBALS['TL_LANG']['tl_apply']['beschreibung'] = array('Beschreibung');

$GLOBALS['TL_LANG']['tl_company']['name_legend'] = 'Name';
$GLOBALS['TL_LANG']['tl_company']['shortname'] = array('Kurzname');
$GLOBALS['TL_LANG']['tl_company']['name'] = array('Name des Unternehmens');
$GLOBALS['TL_LANG']['tl_company']['address_legend'] = 'Addressdaten';
$GLOBALS['TL_LANG']['tl_company']['street'] = array('Strasse');
$GLOBALS['TL_LANG']['tl_company']['number'] = array('Hausnummer');
$GLOBALS['TL_LANG']['tl_company']['zip'] = array('Plz');
$GLOBALS['TL_LANG']['tl_company']['city'] = array('Ort');
$GLOBALS['TL_LANG']['tl_company']['lat'] = array('Latitude','Wenn leer, dann wird die Latitude automatisch von Google erfragt.');
$GLOBALS['TL_LANG']['tl_company']['lon'] = array('Longitude','Wenn leer, dann wird die Longitude automatisch von Google erfragt.');
$GLOBALS['TL_LANG']['tl_company']['contact_legend'] = 'Kontaktdaten';
$GLOBALS['TL_LANG']['tl_company']['contact'] = array('Kontaktperson');
$GLOBALS['TL_LANG']['tl_company']['email'] = array('E-Mail-Adresse');
$GLOBALS['TL_LANG']['tl_company']['phone'] = array('Telefonnummer');
$GLOBALS['TL_LANG']['tl_company']['job_legend'] = 'Ausbildungsstellen';
$GLOBALS['TL_LANG']['tl_company']['restaurant'] = array('Fachmann/-frau für Restaurants und Veranstaltungsgastronomie');
$GLOBALS['TL_LANG']['tl_company']['cook'] = array('Koch/Köchin');
$GLOBALS['TL_LANG']['tl_company']['kitchen'] = array('Fachkraft Küche');
$GLOBALS['TL_LANG']['tl_company']['systemg'] = array('Fachmann/-frau für Systemgastronomie');
$GLOBALS['TL_LANG']['tl_company']['hotelcleaner'] = array('Hotelfachmann/-frau');
$GLOBALS['TL_LANG']['tl_company']['hotelmanager'] = array('Kaufmann/-frau für Hotelmanagement');
$GLOBALS['TL_LANG']['tl_company']['gastro'] = array('Fachkraft für Gastronomie');
$GLOBALS['TL_LANG']['tl_company']['description_legend'] = 'Beschreibung';
$GLOBALS['TL_LANG']['tl_company']['shortdesc'] = array('Kurzbeschreibung');
$GLOBALS['TL_LANG']['tl_company']['description'] = array('Beschreibung');

$GLOBALS['TL_LANG']['tl_member']['job_legend'] = 'Gastroburner / Ausbildungsstellen';
$GLOBALS['TL_LANG']['tl_member']['companyLogo'] = ['Firmen-Logo'];
$GLOBALS['TL_LANG']['tl_member']['lat'] = array('Latitude','Wenn leer, dann wird die Latitude automatisch von Google erfragt.');
$GLOBALS['TL_LANG']['tl_member']['lon'] = array('Longitude','Wenn leer, dann wird die Longitude automatisch von Google erfragt.');
$GLOBALS['TL_LANG']['tl_member']['shortname'] = array('Kurzname', 'Kurzname der Firma');
$GLOBALS['TL_LANG']['tl_member']['restaurant'] = array('Fachmann/-frau für Restaurants und Veranstaltungsgastronomie');
$GLOBALS['TL_LANG']['tl_member']['cook'] = array('Koch/Köchin');
$GLOBALS['TL_LANG']['tl_member']['kitchen'] = array('Fachkraft Küche');
$GLOBALS['TL_LANG']['tl_member']['systemg'] = array('Fachmann/-frau für Systemgastronomie');
$GLOBALS['TL_LANG']['tl_member']['hotelcleaner'] = array('Hotelfachmann/-frau');
$GLOBALS['TL_LANG']['tl_member']['hotelmanager'] = array('Kaufmann/-frau für Hotelmanagement');
$GLOBALS['TL_LANG']['tl_member']['gastro'] = array('Fachkraft für Gastronomie');
$GLOBALS['TL_LANG']['tl_member']['show_in_frontend'] = array('Auf der Webseite ausgeben');
$GLOBALS['TL_LANG']['tl_member']['website'] = array('Internet-Adresse (http://www.beispiel.de)');
$GLOBALS['TL_LANG']['tl_member']['description'] = array('Beschreibung der Firma');
$GLOBALS['TL_LANG']['tl_member']['shortdesc'] = array('Kurzbeschreibung');
